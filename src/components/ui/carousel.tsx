      ...
      <div className='flex gap-2'>
        {scrollSnaps.map((_: any, index: React.Key | null | undefined) => (
          <button
            ...
            className={`relative inline-flex p-0 m-0 w-8 h-1.5`}
          >
            <div className='bg-white/40 h-1.5 rounded-full w-8'></div>
            {index === selectedIndex && (
              <AnimatePresence mode='wait'>
                <motion.div
                  ...
                  className={cn(
                    'absolute z-[3] w-full h-full left-0 top-0 bg-white rounded-full',
                    activeclass
                  )}
                />
              </AnimatePresence>
