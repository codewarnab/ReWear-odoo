...
export default function AddressAutocompleteMap({
    ...
    const onDrag = async () => {
        if (markerRef.current) {
            ...
            // Fetch address for the dragged coordinates
            const apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY;
            if (apiKey) {
                setIsLoadingAddress(true)
                try {
                    const address = await coordinatesToAddress({ lat: lngLat.lat, lng: lngLat.lng }, apiKey)
                    setQuery(address)
                    onCoordinatesChange(lngLat.lat.toFixed(6), lngLat.lng.toFixed(6), address)
                } catch (error) {
    ...
    const handleCoordinateChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!isNaN(lat) && !isNaN(lng)) {
            ...
            if (apiKey) {
                setIsLoadingAddress(true)
                try {
                    const address = await coordinatesToAddress({ lat, lng }, apiKey)
                    setQuery(address)
                    onCoordinatesChange(lat.toFixed(6), lng.toFixed(6), address)
                } catch (error) {
                    console.error('Error fetching address:', error)
                    onCoordinatesChange(lat.toFixed(6), lng.toFixed(6), `${lat.toFixed(4)}, ${lng.toFixed(4)}`)
                } finally {
                    setIsLoadingAddress(false)
                }
            }
